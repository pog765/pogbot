...
def alg2():
	#пампадампа
	#выгружаем таблицу из файла docs
	#поля таблицы
	#пара, ордер текущий,количество монет,количесво битка,  процент,номер сделки, тип сделки(основной/обратный), придельное значение цены ордера 
	#берем ордера из истории 
	#если ордер сработал то ставим обратный ордер 
	#берем текушие цены монет
	
	qq=float (pl.tick('all'))
	#прибавляем или убавляем к текушей цене процент 
	
	#если цена изменилась более чем процент деленный на 5 
	#закрываем ордер из таблицы 
	#открываем ордер с новой ценой 
